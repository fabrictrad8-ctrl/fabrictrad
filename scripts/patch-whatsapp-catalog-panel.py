...
def once(old: str, new: str):
    global text
    if old not in text:
        raise SystemExit(f'Expected WhatsAppCatalogPanel text missing: {old[:160]!r}')
...
anchor = """const money = (value: unknown) => {
...
# Change button label and disabled explanation.
once(
...
"""
insert_anchor = """      {!status?.channelReady && ("""
if identity_section.strip() not in text:
    if insert_anchor not in text:
        raise SystemExit('Could not locate WhatsApp panel status banner anchor.')
    text = text.replace(insert_anchor, identity_section + "\n" + insert_anchor, 1)
