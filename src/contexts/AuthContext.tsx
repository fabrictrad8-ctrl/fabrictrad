...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const verifyEmailOtp = async (email: string, token: string) => {
    ...
    if (normalizedEmail === ADMIN_EMAIL) {
      const claimResponse = await fetch('/api/auth/claim-admin', {
        method: 'POST',
        credentials: 'same-origin',
        cache: 'no-store',
      });
      if (!claimResponse.ok) {
        const fallback = await supabase.rpc('claim_fabrictrad_admin');
        if (fallback.error) {
          const payload = await claimResponse.json().catch(() => null);
          throw new Error(payload?.error || 'Administrator access is not configured on the server.');
        }
      }
