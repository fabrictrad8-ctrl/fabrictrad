...
export function validateSellerPayout(body: Record<string, unknown>): SellerPayoutInput {
  ...
  if (!/^[A-Z]{3}P[A-Z][0-9]{4}[A-Z]$/.test(result.contactPan)) throw new Error('Enter the authorised representative’s valid individual PAN.');
  if (!/^[0-9]{5,35}$/.test(result.accountNumber) || result.accountNumber !== result.confirmAccountNumber) throw new Error('Enter matching full bank account numbers. Masked account numbers cannot be used.');
