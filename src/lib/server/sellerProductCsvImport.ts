...
export async function importSellerProductsCsv(request: NextRequest) {
  const contentLength = Number(request.headers.get('content-length') || 0);
  if (contentLength > MAX_FILE_BYTES) return json({ error: 'CSV is larger than the 2 MB upload limit.' }, 413);
  ...
  const { data: profile, error: profileError } = await supabase
    .from('user_profiles')
    .select('role,can_sell,city,state')
    .eq('id', user.id)
    .maybeSingle();
  ...
  if (Buffer.byteLength(text, 'utf8') > MAX_FILE_BYTES) return json({ error: 'CSV is larger than the 2 MB upload limit.' }, 413);
  ...
  if (rows.length - 1 > MAX_ROWS) return json({ error: `A single import can contain at most ${MAX_ROWS} product rows.` }, 413);

  const headers = rows[0].map(normalizeHeader);
  const duplicates = headers.filter((header, index) => headers.indexOf(header) !== index);
  if (duplicates.length) return json({ error: `Duplicate CSV columns: ${[...new Set(duplicates)].join(', ')}.` }, 400);
  ...
  rows.slice(1).forEach((values, rowIndex) => {
    ...
    const gsm = optionalNonNegativeNumber(source.gsm || '');
    const width = optionalNonNegativeNumber(source.width || '');
    ...
    const retailMax = optionalNonNegativeNumber(source.retail_store_max_quantity || '');
    const endUserMin = finiteNumber(source.end_user_min_quantity || '', 1);
    const endUserMax = optionalNonNegativeNumber(source.end_user_max_quantity || '');
    ...
    if (sku && seenSkus.has(sku)) problems.push('SKU is duplicated in this CSV');
    if (price === null || price <= 0) problems.push('price must be greater than 0');
    if (available === null || available < 0) problems.push('available must be 0 or greater');
    if (moq === null || moq < 1) problems.push('moq must be at least 1');
    if (minStock === null || minStock < 0) problems.push('min_stock must be 0 or greater');
    if (Number.isNaN(gsm)) problems.push('gsm must be a non-negative number');
    if (Number.isNaN(width)) problems.push('width must be a non-negative number');
    if (dispatchDays === null || !Number.isInteger(dispatchDays) || dispatchDays < 0) problems.push('dispatch_days must be a non-negative whole number');
    if (!ALLOWED_STATUSES.has(status)) problems.push('status must be draft, active, or archived');
    if (!ALLOWED_SALE_CHANNELS.has(saleChannel)) problems.push('sale_channel must be b2b, retail, or both');
    if (!validOptionalUrl(imageUrl)) problems.push('image_url must be a valid http:// or https:// URL');
    if (retailMin === null || retailMin < 0) problems.push('retail_store_min_quantity must be 0 or greater');
    if (Number.isNaN(retailMax) || (retailMax !== null && retailMin !== null && retailMax < retailMin)) problems.push('retail_store_max_quantity must be blank or at least the minimum');
    if (endUserMin === null || endUserMin < 0) problems.push('end_user_min_quantity must be 0 or greater');
    if (Number.isNaN(endUserMax) || (endUserMax !== null && endUserMin !== null && endUserMax < endUserMin)) problems.push('end_user_max_quantity must be blank or at least the minimum');

    if (problems.length) {
      errors.push({ row: csvRow, sku, message: problems.join('; ') });
      return;
    }
    ...
    records.push({
      ...
      origin_city: (source.origin_city || '').trim() || profile?.city || null,
      origin_state: (source.origin_state || '').trim() || profile?.state || null,
      status,
  ...
  if (!records.length) {
    return json({ error: 'No valid product rows were found.', imported: 0, rejected: errors.length, total: rows.length - 1, errors }, 422);
  }

  const { error: upsertError } = await supabase.from('seller_products').upsert(records, { onConflict: 'seller_id,sku' });
  if (upsertError) return json({ error: `Products could not be saved: ${upsertError.message}` }, 500);
  ...
  return json({
    ...
    message: errors.length
      ? `${records.length} product rows imported; ${errors.length} rejected.`
      : `${records.length} product rows imported successfully.`,
  });
}
