...
export async function fetchOrderRouteTransfers(orderId: string): Promise<unknown> {
  if (!/^order_[a-zA-Z0-9]+$/.test(orderId)) throw new RouteSetupError('Invalid payment order reference.');
  // Route exposes transfers by expanding the order, not through an /orders/:id/transfers endpoint.
  // https://razorpay.com/docs/api/payments/route/fetch-transfer-order/
  const order = await razorpayRequest<{ id?: string; transfers?: unknown }>(`/v1/orders/${encodeURIComponent(orderId)}?expand%5B%5D=transfers`);
  if (order.id !== orderId) throw new RouteSetupError('Razorpay returned a different payment order.', 503, 'PAYOUT_IDENTITY_MISMATCH');
