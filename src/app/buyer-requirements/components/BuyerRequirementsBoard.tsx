...
export default function BuyerRequirementsBoard() {
  ...
  // Reference photos cannot be uploaded or stored yet (no image field on
  // buyer_requirements and no image-matching backend exists), so this only
  // records the filename as a text note the buyer can see and edit — it
  // never claims to have scanned or matched the image against real sellers.
  const noteReferenceImage = (fileName: string) => {
    setReferenceImageName(fileName);
    setForm((current) => ({
      ...current,
      description: current.description
        ? current.description
        : `Reference photo on hand (${fileName}) — will share it once a seller responds in chat.`,
    }));
            ...
            <div>
              <div className="mb-2 flex items-center gap-2">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-secondary/10">
                  <Icon name="PhotoIcon" size={18} className="text-secondary" />
                </div>
                <div>
                  <h2 className="text-base font-800 text-foreground">Have a reference photo?</h2>
                  <p className="text-xs text-muted-foreground">
                    Post a requirement and share it with sellers directly in chat.
                  </p>
                </div>
              </div>
              <p className="text-sm leading-relaxed text-muted-foreground">
                FabricTrad does not scan or auto-match images yet. Selecting a photo below opens the
                requirement form so you can describe it and send it to sellers once you are chatting.
              </p>
            </div>
            ...
            <div className="rounded-xl border border-dashed border-border bg-muted/40 p-4">
              <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl bg-card px-4 py-5 text-center transition-colors hover:bg-muted">
                ...
                <span className="text-sm font-800 text-foreground">
                  {referenceImageName || 'Choose a reference photo'}
                </span>
                <span className="text-xs text-muted-foreground">
                  Starts a new requirement post noting you have this photo ready
                </span>
                <input
                  type="file"
                  accept="image/*"
                  ...
                  onChange={(event) => {
                    ...
                    if (file) noteReferenceImage(file.name);
                  }}
                />
              </label>
                ...
                <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-border bg-muted px-3 py-3 transition-colors hover:border-primary/60">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-700 text-foreground">
                      {referenceImageName || 'Note that you have a reference photo'}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      This only records the filename in your description — the photo itself is shared with a seller directly in chat, not uploaded here.
                    </p>
                  </div>
                  <input
                    type="file"
                    ...
                    onChange={(event) => {
                      ...
                      setForm((current) => ({
                        ...current,
                        description: current.description
                          ? `${current.description}\n(Reference photo on hand: ${file.name})`
                          : `Reference photo on hand (${file.name}) — will share it once a seller responds in chat.`,
                      }));
                    }}
                  />
                </label>
