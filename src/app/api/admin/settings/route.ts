...
export async function GET() {
  ...
  const [policies, eligibleSellers, readySellers, invoiceTotal, acceptedInvoices, credentials] = await Promise.all([
    ...
    admin.from('seller_profiles').select('*', { count: 'exact', head: true }).eq('is_active', true).eq('settlement_eligible', true).not('razorpay_linked_account_id', 'is', null).neq('razorpay_linked_account_id', ''),
    admin.from('seller_tax_invoices').select('*', { count: 'exact', head: true }),
    admin.from('seller_tax_invoices').select('*', { count: 'exact', head: true }).eq('email_status', 'sent'),
  ...
  const emailConfigured = Boolean(process.env.RESEND_API_KEY?.trim() || process.env.SMTP_PASSWORD?.startsWith('re_'));
  return json({
    ...
    payouts: { eligibleSellers: eligibleSellers.count || 0, missingAccounts: Math.max(0, (eligibleSellers.count || 0) - (readySellers.count || 0)) },
