...
export async function POST(request: NextRequest) {
  ...
  if (activePayment?.id) {
    ...
    if (providerOrderId.startsWith('pending_')) {
      ...
      if (
        ...
        const transfers = await fetchOrderRouteTransfers(providerOrderId).catch(() => null);
      ...
      return json({
        error: 'The earlier checkout could not be confirmed. Contact FabricTrad before trying another payment.',
        code: 'PAYMENT_SESSION_REVIEW_REQUIRED',
      }, 409);
    }
  }
  ...
  const transfers = await fetchOrderRouteTransfers(provider.id).catch(() => null);
