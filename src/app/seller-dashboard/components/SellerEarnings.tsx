...
const netSellerPayable = (payment: PaymentRow) => {
  if (payment.capturedAmount <= 0) return 0;
...
export default function SellerEarnings() {
  ...
  const load = useCallback(async () => {
    ...
    const catalogPromise = catalogIds.length
      ? supabase
          ...
          .select(`catalog_order_id,${fields}`)
    ...
    const mapPayment = (row: Record<string, unknown>, orderType: 'catalog' | 'bulk'): PaymentRow => ({
      id: String(row.id),
      orderId: String(orderType === 'catalog' ? row.catalog_order_id : row.bulk_order_id),
      orderType,
      amount: Number(row.amount || 0),
      // Never fall back to the requested/authorised amount. Earnings begin only
      // when our signed Razorpay verification/webhook stores captured_amount.
    ...
    const mapped = [
      ...(catalogResult.data || []).map((row) => mapPayment(row as Record<string, unknown>, 'catalog')),
      ...(bulkResult.data || []).map((row) => mapPayment(row as Record<string, unknown>, 'bulk')),
  ...
  const captured = useMemo(
    () =>
      payments.filter(
        (payment) =>
          payment.capturedAmount > 0 && capturedStatuses.has(payment.status.toLowerCase())
      ),
    [payments]
  );
  const grossCaptured = captured.reduce((sum, payment) => sum + payment.capturedAmount, 0);
  ...
  const monthRows = useMemo(() => {
    ...
    for (let offset = 5; offset >= 0; offset -= 1) {
      ...
      rows.set(monthKey(date.toISOString()), {
    ...
    captured.forEach((payment) => {
      ...
      if (row) row.earnings += netSellerPayable(payment);
    });
    return [...rows.values()];
  }, [captured]);
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Earnings are recognised only after Razorpay confirms payment capture. Initiated and authorised payments are excluded.
          </p>
      ...
      <div className="mb-6 flex gap-1 overflow-x-auto rounded-xl bg-muted p-1">
        {tabs.map((tab) => (
          <button key={tab.key} type="button" onClick={() => setActiveSection(tab.key)} className={`flex items-center gap-1.5 whitespace-nowrap rounded-lg px-3 py-2 text-xs font-700 ${activeSection === tab.key ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground'}`}>
      ...
      {activeSection === 'overview' && (
        <div className="space-y-5">
          <section className="rounded-2xl border border-border bg-card p-5">
            <h2 className="mb-4 text-sm font-800 text-foreground">Captured seller payable by month</h2>
            <ResponsiveContainer width="100%" height={230}>
              <BarChart data={monthRows} barSize={18}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                <XAxis dataKey="month" tick={{ fontSize: 10, fill: 'var(--muted-foreground)' }} tickLine={false} axisLine={false} />
                <YAxis tick={{ fontSize: 10, fill: 'var(--muted-foreground)' }} tickLine={false} axisLine={false} tickFormatter={(value) => `₹${Math.round(Number(value) / 1000)}K`} />
                <Tooltip formatter={(value: number) => [money(value), 'Captured seller payable']} contentStyle={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, fontSize: 12 }} />
            ...
            {!captured.length && (
              <div className="mt-3 rounded-xl border border-dashed border-border bg-muted/30 p-4 text-center">
                <p className="text-sm font-800">No captured payments yet</p>
                <p className="mt-1 text-xs text-muted-foreground">Initiated or authorised Razorpay orders do not count as earnings. This chart starts only after a real capture.</p>
              </div>
            )}
          </section>
