  ...
  return (
    <div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        ].map(([label, value, icon, color]) => <div key={String(label)} className="ft-tile"><Icon name={String(icon)} size={20} className={String(color)} /><span className={`ft-tile-value ${color}`}>{value}</span><span className="ft-tile-label normal-case tracking-normal">{label}</span></div>)}
      </div>
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        ...
        {scopedDisputes.length ? <div className="mt-4 divide-y divide-border">{scopedDisputes.slice(0, 12).map((dispute) => <div key={dispute.id} className="flex flex-wrap items-center justify-between gap-3 py-3"><div><p className="text-sm font-700 text-foreground">{dispute.dispute_type || 'Order dispute'}</p><p className="mt-1 text-xs text-muted-foreground">Opened {new Date(dispute.created_at).toLocaleString('en-IN')}</p></div><div className="text-right"><span className={pillClassForStatus(dispute.status || 'open')}>{pillLabel(dispute.status || 'open')}</span>{Number(dispute.requested_refund_amount || 0) > 0 && <p className="mt-1 text-xs text-error">₹{Number(dispute.requested_refund_amount).toLocaleString('en-IN')} requested</p>}</div></div>)}</div> : <div className="mt-4 rounded-xl border border-dashed border-border bg-muted/30 p-6 text-center text-xs text-muted-foreground">No disputes in this period.</div>}
      </section>
    </div>
  );
}
