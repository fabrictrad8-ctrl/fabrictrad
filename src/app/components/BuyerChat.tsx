...
export default function BuyerChat({ contextId, contextTitle, sellerName, sellerAvatar, onClose }: BuyerChatProps) {
    ...
    return (
      <div className="fixed bottom-4 right-4 z-50 w-80 bg-card border border-border rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-5 text-center">
          ...
          <p className="text-xs text-muted-foreground mb-4">You need to be logged in as a buyer to chat with sellers.</p>
          <Link href="/login?role=buyer" className="btn-primary w-full py-2.5 text-sm rounded-xl block text-center">
            Sign In as Buyer
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="fixed bottom-4 right-4 z-50 w-80 bg-card border border-border rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-5 text-center">
          ...
          <p className="text-xs text-muted-foreground">This chat is for buyers. Sellers manage conversations from the Seller Dashboard → Inbox.</p>
        </div>
      </div>
    );
  }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setAttachedFile({ name: file.name, type: fileType, preview: isImage ? URL.createObjectURL(file) : undefined });
  };
    ...
    <div className="fixed bottom-4 right-4 z-50 flex flex-col shadow-2xl rounded-2xl overflow-hidden border border-border bg-card"
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-secondary to-primary text-white shrink-0">
        <div className="w-8 h-8 rounded-full overflow-hidden bg-white/20 shrink-0">
          <AppImage src={sellerAvatar} alt={`${sellerName} seller profile photo`} width={32} height={32} className="object-cover" />
        </div>
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setIsMinimized((m) => !m)} className="p-1 hover:bg-white/20 rounded-lg">
      ...
      {!isMinimized && (
        <>
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-success/10 border-b border-success/20">
            ...
            <p className="text-xs text-success font-600">Secure buyer chat · No contact info sharing</p>
          </div>
              ...
              return (
                <div key={msg.id} className={`flex gap-2 ${msg.isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`flex flex-col gap-1 max-w-[75%] ${msg.isMe ? 'items-end' : 'items-start'}`}>
                    {!msg.isMe && <p className="text-xs text-muted-foreground px-1">{msg.senderName}</p>}
                    <div className={`rounded-2xl px-3 py-2 text-sm ${msg.isMe ? 'bg-primary text-white rounded-tr-sm' : 'bg-card border border-border text-foreground rounded-tl-sm'}`}>
                      {msg.text && <p className="leading-relaxed">{msg.text}</p>}
                      {msg.fileName && (
                        <div className={`flex items-center gap-2 mt-1 p-2 rounded-lg ${msg.isMe ? 'bg-white/20' : 'bg-muted'}`}>
                          <Icon name={msg.fileType === 'image' ? 'PhotoIcon' : 'DocumentIcon'} size={14} className={msg.isMe ? 'text-white' : 'text-muted-foreground'} />
                          <span className={`text-xs truncate max-w-[140px] ${msg.isMe ? 'text-white' : 'text-foreground'}`}>{msg.fileName}</span>
                        </div>
                      )}
                    </div>
          ...
          {blockedWarning && (
            <div className="mx-3 mb-2 p-2 bg-error/10 border border-error/20 rounded-xl flex items-center gap-2">
              ...
              <p className="text-xs text-error">Phone numbers, emails, and external contact details are not allowed.</p>
            </div>
          )}
          ...
          {attachedFile && (
            <div className="mx-3 mb-1 flex items-center gap-2 p-2 bg-primary/10 border border-primary/20 rounded-xl">
              ...
              <button onClick={() => setAttachedFile(null)}><Icon name="XMarkIcon" size={13} className="text-muted-foreground" /></button>
            </div>
          )}

          <div className="p-3 border-t border-border bg-card shrink-0">
            <div className="flex items-end gap-2">
              <button onClick={() => fileInputRef.current?.click()} className="p-2 hover:bg-muted rounded-lg transition-colors shrink-0">
