    ...
    <section className="py-12 md:py-16 bg-muted/40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
          <div>
            <p className="text-xs font-700 text-primary uppercase tracking-widest mb-1">Textile Products</p>
            <h2 className="text-section-title text-foreground">Featured Listings</h2>
          </div>
          ...
          <div className="flex items-center gap-1 bg-card border border-border rounded-xl p-1">
            ...
            <button
              ...
              'text-muted-foreground hover:text-foreground'}`
                ...
                {tab === 'bestseller' ? 'Best Sellers' : tab === 'new' ? 'New Arrivals' : 'Featured'}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
          ...
          <div
            ...
            style={{ animationDelay: `${i * 60}ms` }}>
              ...
              'bg-primary border-primary' : 'bg-white/80 border-border opacity-0 group-hover:opacity-100'}`
              }
            ...
            <div className="absolute top-2 right-2 z-10">
                  {product.badge === 'bestseller' && <span className="tag-bestseller">Best Seller</span>}
                  {product.badge === 'new' && <span className="tag-new">New</span>}
                  {product.badge === 'premium' &&
              <span className="bg-gradient-to-r from-amber-500 to-yellow-400 text-white text-[0.6rem] font-800 px-2 py-0.5 rounded-full uppercase tracking-wide">Premium</span>
              }
                </div>
            }
              ...
              <div className="p-3">
                ...
                <Icon name="ShieldCheckIcon" size={12} className="text-success shrink-0" />
                  ...
                  <span className="text-xs text-muted-foreground truncate">{product.sellerCity}</span>
                  {product.gstReady &&
                <span className="badge-gstin ml-auto shrink-0">GST</span>
                }
                </div>
                ...
                <div className="flex flex-wrap gap-1 mb-2">
                  <span className="text-xs bg-muted text-muted-foreground px-1.5 py-0.5 rounded">{product.gsm}</span>
                  <span className="text-xs bg-muted text-muted-foreground px-1.5 py-0.5 rounded">{product.width}</span>
                </div>
                ...
                <div className="flex items-end justify-between mb-3">
                  <div>
                    <span className="text-base font-800 text-primary">₹{product.price.toLocaleString('en-IN')}</span>
                    <span className="text-xs text-muted-foreground ml-1">{product.unit}</span>
                  </div>
                  <span className="badge-moq">MOQ: {product.moq}{product.moqUnit}</span>
                </div>

                {/* Dispatch */}
                <div className="flex items-center gap-1 mb-3">
                  <Icon name="TruckIcon" size={12} className="text-muted-foreground" />
                  <span className="text-xs text-muted-foreground">Dispatch in {product.dispatch}</span>
                </div>
