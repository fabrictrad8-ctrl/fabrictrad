...
export default function FabricDrapeViewer() {
  ...
  // When AI response arrives, finish generating
  useEffect(() => {
  ...
  const handleGenerate = async (bypassPaywall = false) => {
    if (!bypassPaywall && freeRemaining <= 0) {
      setShowPaywall(true);
      return;
    }
    ...
    try {
      // Convert fabric image to base64
      const fabricBase64 = await imageUrlToBase64(selectedFabric.src);
      // Convert model/selfie image to base64
      const modelBase64 = await imageUrlToBase64(modelSrc);

      const drapeStyle = photoSource === 'model' ? selectedPose.description : 'the person in the photo';
      const blendDesc = settings.blend === 'multiply' ? 'layered with multiply blend' : settings.blend === 'overlay' ? 'overlaid with overlay blend' : settings.blend === 'screen' ? 'applied with screen blend' : 'placed normally';

      const messages = [
      {
        role: 'system' as const,
        content: `You are an expert fashion AI specializing in fabric draping visualization. 
          Analyze the provided fabric and model/person images and generate a detailed, realistic draping visualization description.
          Be specific about how the fabric would look when draped on the person — describe texture, fall, color interaction, and styling.
          Keep your response concise (3-4 sentences) and visually descriptive. End with one styling tip.`
      },
      {
        role: 'user' as const,
        content: [
        {
          type: 'text' as const,
          text: `Please analyze how this fabric would look when draped on the model/person. 
              Draping style: ${drapeStyle}. 
              Fabric view: ${selectedFabric.name}. 
              Blend mode applied: ${blendDesc}, opacity ${settings.opacity}%, scale ${settings.scale}%.
              Describe the realistic draping result — how the fabric falls, its texture appearance on the body, color harmony, and overall look.`
        },
        {
          type: 'image_url' as const,
          image_url: { url: fabricBase64, detail: 'high' as const }
        },
        {
          type: 'image_url' as const,
          image_url: { url: modelBase64, detail: 'high' as const }
        }]

      }];


      sendMessage(messages, { max_completion_tokens: 300 });
    } catch {
      setIsGenerating(false);
    ...
    <div className="bg-card rounded-2xl border border-border overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">GPT-4o Vision · On model · Upload photo · Camera selfie</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {freeRemaining > 0 ?
        ...
        <div className="flex items-center gap-2 mt-3">
          ...
          <React.Fragment key={s}>
              ...
              className={`flex items-center gap-1.5 text-xs font-600 transition-colors ${step === s ? 'text-primary' : 'text-muted-foreground'}`}>
                ...
                <span
                className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-800 ${step === s ? 'bg-primary text-white' : 'bg-muted text-muted-foreground'}`}>
                
                  {i + 1}
                </span>
                <span className="hidden sm:inline capitalize">
                  {s === 'select' ? 'Select' : s === 'preview' ? 'Adjust' : 'AI Result'}
                </span>
              </button>
              {i < 2 &&
            <div
              className={`flex-1 h-px ${i < ['select', 'preview', 'result'].indexOf(step) ? 'bg-primary' : 'bg-border'}`} />
      ...
      <div className="p-4">
        ...
        <div className="space-y-5">
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-5 h-5 rounded-full bg-primary text-white text-xs font-800 flex items-center justify-center">
                  1
                </div>
            ...
            <div
              ...
              <>
                      <div className="w-12 h-12 rounded-lg overflow-hidden shrink-0">
                        <img
                    src={uploadedPhoto}
                    alt="Uploaded photo for fabric draping"
            ...
            <div className="space-y-3">
              ...
              <div className="relative rounded-xl overflow-hidden">
                      <img
                  src={uploadedPhoto}
                  alt="Captured selfie for fabric draping"
                  className="w-full rounded-xl"
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 p-3 bg-primary/5 border border-primary/20 rounded-xl">
              ...
              <p className="text-xs text-muted-foreground">
                <span className="font-700 text-primary">GPT-4o Vision</span> will analyze both images and generate a realistic draping description
              </p>
            </div>
        ...
        <div className="space-y-4">
            ...
            {/* AI Analysis */}
            {aiAnalysis &&
          <div className="p-4 bg-gradient-to-br from-primary/5 to-secondary/5 border border-primary/20 rounded-2xl">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-6 h-6 rounded-lg bg-primary/10 flex items-center justify-center">
                    <Icon name="SparklesIcon" size={13} className="text-primary" />
                  </div>
                  <p className="text-xs font-700 text-primary">AI Drape Analysis</p>
                  <span className="text-xs text-muted-foreground ml-auto">GPT-4o Vision</span>
                </div>
            ...
            <div className="bg-card rounded-2xl border border-border p-6 max-w-sm w-full shadow-xl">
              <div className="text-center mb-5">
                ...
                <p className="text-sm text-muted-foreground">
                  You've used your {FREE_QUOTA} free drapes today. Upgrade for unlimited access.
                </p>
              </div>
