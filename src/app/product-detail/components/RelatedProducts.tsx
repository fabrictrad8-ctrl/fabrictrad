  ...
  return (
    <div className="mt-10">
      <div className="flex items-center justify-between mb-5">
        ...
        <Link href="/marketplace" className="text-sm font-600 text-primary hover:underline">View All →</Link>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {related?.map((p) =>
        <Link key={p?.name} href="/product-detail" className="product-card border border-border group block">
            <div className="aspect-square overflow-hidden bg-muted">
              <AppImage src={p?.image} alt={p?.alt} width={200} height={200} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
            </div>
            <div className="p-3">
              <h3 className="text-xs font-700 text-foreground line-clamp-2 mb-1 group-hover:text-primary transition-colors">{p?.name}</h3>
              ...
              <div className="flex items-end justify-between">
                <span className="text-sm font-800 text-primary">₹{p?.price?.toLocaleString('en-IN')}<span className="text-xs text-muted-foreground font-400">/mtr</span></span>
