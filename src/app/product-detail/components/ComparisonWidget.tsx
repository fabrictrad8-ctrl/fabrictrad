...
},
{
  ...
  dispatch: '3-5 days',
  acceptance: '89%',
  image: "https://img.rocket.new/generatedImages/rocket_gen_img_1ed6be656-1784319275674.png",
...
},
{
  ...
  dispatch: '4-6 days',
  acceptance: '82%',
  image: "https://img.rocket.new/generatedImages/rocket_gen_img_113bb05f9-1784319275951.png",
...
const compareFields = [
{ key: 'price', label: 'Price/mtr', format: (v: number) => `₹${v}`, best: 'low' },
{ key: 'moq', label: 'Min. Order', format: (v: number) => `${v} mtrs`, best: 'low' },
{ key: 'rating', label: 'Rating', format: (v: number) => `${v} ★`, best: 'high' },
  ...
  return (
    <div className="bg-card rounded-2xl border border-border p-5 mt-6">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        ...
        <div key={alt.id} className={`rounded-xl border p-3 text-center relative ${alt.isCurrent ? 'border-primary bg-primary/5' : 'border-border'}`}>
            {alt.isCurrent &&
          <span className="absolute -top-2 left-1/2 -translate-x-1/2 text-xs font-700 bg-primary text-white px-2 py-0.5 rounded-full whitespace-nowrap">Current</span>
          }
            <div className="w-12 h-12 rounded-lg overflow-hidden bg-muted mx-auto mb-2">
              <AppImage src={alt.image} alt={alt.alt} width={48} height={48} className="w-full h-full object-cover" />
            </div>
      ...
      <div className="space-y-2">
        {compareFields.slice(0, expanded ? compareFields.length : 3).map((field) => {
          const values = alternatives.map((a) => (a as any)[field.key]);
          const numericVals = values.filter((v) => typeof v === 'number') as number[];
          const bestVal = field.best === 'low' ? Math.min(...numericVals) : field.best === 'high' ? Math.max(...numericVals) : null;

          return (
            <div key={field.key} className="grid grid-cols-3 gap-3 items-center">
              {alternatives.map((alt, i) => {
                const val = (alt as any)[field.key];
                const isBest = bestVal !== null && val === bestVal;
                return (
                  <div key={alt.id} className={`rounded-lg p-2 text-center ${isBest ? 'bg-success/10 border border-success/20' : 'bg-muted'}`}>
                    {i === 0 &&
                    <p className="text-xs text-muted-foreground mb-0.5 hidden sm:block">{field.label}</p>
                    }
                    <p className={`text-xs font-700 ${isBest ? 'text-success' : 'text-foreground'}`}>
                      {field.format(val)}
                    </p>
      ...
      <div className="grid grid-cols-3 gap-3 mt-3">
        ...
        <div key={alt.id}>
            {!alt.isCurrent &&
          <Link href="/product-detail" className="btn-secondary w-full py-2 text-xs rounded-xl flex items-center justify-center gap-1">
