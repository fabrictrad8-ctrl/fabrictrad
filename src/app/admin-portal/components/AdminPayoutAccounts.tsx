...
export default function AdminPayoutAccounts() {
  ...
  return <section className="rounded-2xl border border-border bg-card p-6"><h2 className="text-lg font-800">Seller payout verification</h2><p className="mt-2 text-sm text-muted-foreground">Seller bank activation is checked with Razorpay before every checkout. New payments allocate 90% to the seller and an all-inclusive 10% to FabricTrad.</p>
    ...
    <div className="mt-4 space-y-4">{sellers.map(s => <article key={s.sellerId} className="rounded-xl border border-border p-4"><div className="flex flex-wrap justify-between gap-3"><div><h3 className="font-700">{s.name}</h3><p className="mt-1 text-sm">Seller: {s.sellerStatus} · Payout: {(s.account?.activation_status || 'not connected').replaceAll('_', ' ')}</p><p className="mt-1 text-sm text-muted-foreground">{s.account?.bank_last4 ? `Bank •••• ${s.account.bank_last4} · ${s.account.bank_ifsc}` : 'Seller needs to enter their full bank details in Earnings → Payout account.'}</p>{s.account?.linked_account_id && <p className="mt-1 break-all text-xs text-muted-foreground">{s.account.linked_account_id} · {s.account.product_id || 'Route product pending'}</p>}</div><button type="button" disabled={busy || !s.account?.product_id} onClick={() => void update({ sellerId: s.sellerId, action: 'refresh' })} className="min-h-11 rounded-xl border border-border px-3 text-sm disabled:opacity-40">Check Razorpay</button></div>
