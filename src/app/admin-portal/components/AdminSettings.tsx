    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          Marketplace-wide configuration · Super Admin only
        </p>
      </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Applied to all seller transactions. Current: {commissionRate}%
              </p>
            </div>
            <div>
              <label className="text-xs font-700 text-foreground block mb-1.5">
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-700 text-foreground">
                  Auto-approve listings for verified sellers
                </p>
                <p className="text-xs text-muted-foreground">
                  Skip manual review for trusted sellers
                </p>
              </div>
              <button
              ...
              >
                <span
                  className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${autoApprove ? 'translate-x-5' : 'translate-x-0.5'}`}
                />
              </button>
            </div>
            <div>
              <label className="text-xs font-700 text-foreground block mb-1.5">
                Seller Response Timeout (hours)
              </label>
              <input
                type="number"
                defaultValue="24"
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                Orders auto-cancelled if seller doesn't respond within this time
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card rounded-2xl border border-border p-5">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-700 text-foreground">
                  Require OTP for all registrations
                </p>
              ...
              >
                <span
                  className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${otpEnabled ? 'translate-x-5' : 'translate-x-0.5'}`}
                />
              </button>
            </div>
            <div>
            ...
            {[
              ...
              >
                <div className="flex items-center gap-2">
                  <Icon
                    name={integration.icon as 'CreditCardIcon'}
                    size={16}
                ...
                <span
                  className={`text-xs font-600 border rounded-full px-2 py-0.5 ${integration.status === 'Connected' ? 'bg-success/10 text-success border-success/20' : 'bg-amber-50 text-amber-700 border-amber-200'}`}
                >
                  {integration.status}
                </span>
              </div>
