...
const configPill = (configured: boolean, onLabel: string, offLabel: string) => (
  <span className={pillClassForStatus(configured ? 'verified' : 'additional_docs_required')}>{configured ? onLabel : offLabel}</span>
);
  ...
  return <div className="max-w-5xl space-y-6">
    ...
    {settings && <>
      <div className="grid gap-4 md:grid-cols-2">
        <section className="rounded-2xl border border-border bg-card p-6"><h2 className="text-lg font-800">Payments & seller payouts</h2><p className="mt-3 flex items-center gap-2 text-base">Payment credentials: {configPill(settings.payments.configured, settings.payments.mode.toUpperCase(), 'Missing')}</p><p className="mt-2 text-sm text-muted-foreground">{settings.payouts.eligibleSellers} eligible sellers · {settings.payouts.missingAccounts} without a Razorpay-verified payout bank.</p>{settings.payouts.missingAccounts > 0 && <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-900">Complete each seller’s Razorpay linked-account setup before expecting settlement to their bank.</p>}<Link href="/admin-portal?tab=sellers" className="mt-4 inline-flex min-h-11 items-center text-sm font-700 text-primary underline">Review sellers</Link></section>
        <section className="rounded-2xl border border-border bg-card p-6"><h2 className="text-lg font-800">Invoices & email</h2><p className="mt-3 flex items-center gap-2 text-base">Email service: {configPill(settings.invoices.emailConfigured, 'Key configured', 'Setup required')}</p><p className="mt-2 text-sm text-muted-foreground">{settings.invoices.awaitingEmail} awaiting email · {settings.invoices.providerAccepted} accepted by the email provider.</p><p className="mt-4 text-sm text-muted-foreground">Provider acceptance does not confirm inbox delivery. Review pending invoices and verify the sending domain with your email provider.</p><Link href="/admin-portal?tab=orders" className="mt-4 inline-flex min-h-11 items-center text-sm font-700 text-primary underline">Review orders and invoice delivery</Link></section>
        <section className="rounded-2xl border border-border bg-card p-6"><h2 className="text-lg font-800">Seller WhatsApp uploads</h2><p className="mt-3 flex items-center gap-2 text-base">Channel: {configPill(settings.whatsapp.channelConfigured, 'Configured', 'Setup required')}</p><p className="mt-2 flex items-center gap-2 text-sm text-muted-foreground">Callback secret: {configPill(settings.whatsapp.callbackSecretConfigured, 'Configured on the server', 'Missing')}</p><p className="mt-4 text-sm text-muted-foreground">Use the same webhook secret on the Gupshup callback and the server, then verify a seller upload. WhatsApp is reserved for sellers; buyer messaging is disabled.</p></section>
        <section className="rounded-2xl border border-border bg-card p-6"><h2 className="text-lg font-800">Virtual Drape</h2><p className="mt-3 flex items-center gap-2 text-base">Image provider: {configPill(settings.drape.configured, 'Key configured', 'Setup required')}</p><p className="mt-4 text-sm text-muted-foreground">Generation also requires available provider credits. The current experience produces AI images; live 3D body tracking and a full 3D try-on service are not ready.</p></section>
      </div>
