  ...
  return <div className="max-w-5xl space-y-6">
    ...
    {settings && <>
      <div className="grid gap-4 md:grid-cols-2">
        <section className="rounded-2xl border border-border bg-card p-6"><h2 className="text-lg font-800">Payments & seller payouts</h2><p className="mt-3 text-base">Payment credentials: <strong>{settings.payments.configured ? settings.payments.mode.toUpperCase() : 'Missing'}</strong></p><p className="mt-2 text-sm text-muted-foreground">{settings.payouts.eligibleSellers} eligible sellers · {settings.payouts.missingAccounts} without a linked payout account.</p>{settings.payouts.missingAccounts > 0 && <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-900">Complete each seller’s Razorpay linked-account setup before expecting settlement to their bank.</p>}<Link href="/admin-portal?tab=sellers" className="mt-4 inline-flex min-h-11 items-center text-sm font-700 text-primary underline">Review sellers</Link></section>
      ...
      <section className="rounded-2xl border border-border bg-card p-6"><h2 className="text-lg font-800">Recorded commerce policies</h2><dl className="mt-4 divide-y divide-border">{Object.entries(settings.policies).map(([key, value]) => <div key={key} className="flex flex-wrap justify-between gap-3 py-3 text-sm"><dt>{policyLabels[key] || key}</dt><dd className="font-700">{key.endsWith('_rate') ? `${Number(value) * 100}%` : value}</dd></div>)}</dl><p className="mt-4 text-sm text-muted-foreground">Security and provider credentials are managed through the protected deployment configuration. Sellers choose a shipment provider separately for every order and supply its AWB and tracking link.</p></section>
