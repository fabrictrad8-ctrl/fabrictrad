...
const sellersData: Seller[] = [
{
  ...
  responseTime: '1.2 hrs', responseTimeMs: 72, acceptanceRate: 94, orderVolume: 310,
  rating: 4.8, gmv: '₹42L', status: 'verified', joinDate: '12 Jan 2026',
  categories: 'Net Fabric, Embroidered, Georgette', totalReviews: 284, disputeRate: 1.2, lastActive: '2 mins ago'
},
{
  ...
  responseTime: '2.4 hrs', responseTimeMs: 144, acceptanceRate: 88, orderVolume: 189,
  rating: 4.6, gmv: '₹28L', status: 'verified', joinDate: '03 Feb 2026',
  categories: 'Silk, Banarasi, Handloom', totalReviews: 162, disputeRate: 2.1, lastActive: '1 hr ago'
},
{
  ...
  responseTime: '3.8 hrs', responseTimeMs: 228, acceptanceRate: 79, orderVolume: 89,
  rating: 4.1, gmv: '₹12L', status: 'suspended', joinDate: '20 Mar 2026',
  categories: 'Cotton, Linen, Denim', totalReviews: 74, disputeRate: 5.6, lastActive: '3 days ago'
},
{
  id: 'FT-SLR-001102', name: 'Kolkata Silk House', contact: 'Ananya Das',
  city: 'Kolkata, West Bengal', type: 'Wholesaler', gstin: '19FFFKS1234Z1Z2',
  avatar: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=40&h=40&fit=crop&crop=face',
  responseTime: '1.8 hrs', responseTimeMs: 108, acceptanceRate: 91, orderVolume: 241,
  rating: 4.7, gmv: '₹35L', status: 'verified', joinDate: '28 Jan 2026',
  categories: 'Silk, Muslin, Jamdani', totalReviews: 218, disputeRate: 1.8, lastActive: '30 mins ago'
},
{
  id: 'FT-SLR-001445', name: 'Ludhiana Fabric Co.', contact: 'Harpreet Singh',
  city: 'Ludhiana, Punjab', type: 'Manufacturer', gstin: '03GGGFL5678Z2Z3',
  avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=40&h=40&fit=crop&crop=face',
  responseTime: '4.5 hrs', responseTimeMs: 270, acceptanceRate: 72, orderVolume: 134,
  rating: 3.9, gmv: '₹18L', status: 'verified', joinDate: '15 Feb 2026',
  categories: 'Wool, Blended, Suiting', totalReviews: 98, disputeRate: 3.4, lastActive: '5 hrs ago'
}];
...
export default function AdminTopSellers() {
  ...
  const [actionModal, setActionModal] = useState<{seller: Seller;action: 'suspend' | 'deactivate' | 'reactivate';} | null>(null);
  ...
  const handleAction = () => {
    ...
    if (selectedSeller?.id === seller.id) {
      setSelectedSeller((prev) => prev ? { ...prev, status: action === 'reactivate' ? 'verified' : action === 'suspend' ? 'suspended' : 'deactivated' } : null);
    }
  ...
  const getRatingStars = (rating: number) => {
    ...
    <span key={i} className={i < Math.floor(rating) ? 'text-amber-400' : 'text-muted'}>★</span>
    );
  };
    ...
    <div className="relative">
      ...
      <div className="flex items-center gap-2 mb-5 overflow-x-auto scrollbar-thin pb-1">
        ...
        <button
          ...
          sortBy === opt.key ? 'bg-secondary text-white' : 'bg-card border border-border text-muted-foreground hover:border-secondary'}`
          }>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-muted border-b border-border">
                ...
                <th className="text-center px-4 py-3 text-xs font-700 text-muted-foreground">Response Time</th>
                <th className="text-center px-4 py-3 text-xs font-700 text-muted-foreground">Acceptance</th>
                <th className="text-center px-4 py-3 text-xs font-700 text-muted-foreground hidden sm:table-cell">Orders</th>
            ...
            <tbody className="divide-y divide-border">
              ...
              <tr key={seller.id} className="hover:bg-muted/30 transition-colors">
                  <td className="px-4 py-3">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-800 ${
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-xl overflow-hidden bg-muted shrink-0">
                        <AppImage src={seller.avatar} alt={`${seller.contact} profile`} width={32} height={32} className="object-cover" />
                      </div>
                      <div>
                        <p className="text-xs font-700 text-foreground leading-tight">{seller.name}</p>
                        <p className="text-xs text-muted-foreground">{seller.city}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span className={`text-xs font-700 ${seller.responseTimeMs <= 120 ? 'text-success' : seller.responseTimeMs <= 240 ? 'text-warning' : 'text-error'}`}>
                      {seller.responseTime}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <div className="flex flex-col items-center gap-1">
                      <span className={`text-xs font-800 ${seller.acceptanceRate >= 90 ? 'text-success' : seller.acceptanceRate >= 80 ? 'text-warning' : 'text-error'}`}>
                        {seller.acceptanceRate}%
                      </span>
                      <div className="w-12 h-1 bg-muted rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${seller.acceptanceRate >= 90 ? 'bg-success' : seller.acceptanceRate >= 80 ? 'bg-warning' : 'bg-error'}`}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-card rounded-2xl border border-border w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="p-5 border-b border-border flex items-center justify-between">
              ...
              <button onClick={() => setSelectedSeller(null)} className="p-1.5 hover:bg-muted rounded-lg transition-colors">
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="flex items-start gap-4">
                <div className="w-16 h-16 rounded-2xl overflow-hidden bg-muted shrink-0">
                  <AppImage src={selectedSeller.avatar} alt={`${selectedSeller.contact} profile`} width={64} height={64} className="object-cover" />
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="font-800 text-foreground">{selectedSeller.name}</h3>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-600 border ${statusConfig[selectedSeller.status].class}`}>
                      {statusConfig[selectedSeller.status].label}
                    </span>
                  </div>
                  <p className="text-sm text-muted-foreground">{selectedSeller.contact} · {selectedSeller.type}</p>
                  <p className="text-xs text-muted-foreground">{selectedSeller.city} · Joined {selectedSeller.joinDate}</p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
              { label: 'Response Time', value: selectedSeller.responseTime, icon: 'ClockIcon', color: selectedSeller.responseTimeMs <= 120 ? 'text-success' : 'text-warning', bg: 'bg-success/10' },
              { label: 'Acceptance Rate', value: `${selectedSeller.acceptanceRate}%`, icon: 'CheckCircleIcon', color: 'text-primary', bg: 'bg-primary/10' },
              { label: 'Total Orders', value: selectedSeller.orderVolume.toString(), icon: 'ShoppingBagIcon', color: 'text-secondary', bg: 'bg-secondary/10' },
              { label: 'Customer Rating', value: `${selectedSeller.rating}/5`, icon: 'StarIcon', color: 'text-amber-600', bg: 'bg-amber-50' }].
              map((m) =>
              <div key={m.label} className="bg-muted rounded-xl p-3 text-center">
                    <Icon name={m.icon as 'ClockIcon'} size={18} className={`${m.color} mx-auto mb-1`} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                map((f) =>
                <div key={f.label} className="flex items-center justify-between py-1.5 border-b border-border last:border-0">
              ...
              <>
                    <button
                  onClick={() => {setActionModal({ seller: selectedSeller, action: 'suspend' });setSelectedSeller(null);}}
                  className="flex items-center gap-1.5 bg-warning/10 text-warning border border-warning/20 text-xs px-3 py-2 rounded-xl font-600 hover:bg-warning hover:text-white transition-all">
                    ...
                    <button
                  onClick={() => {setActionModal({ seller: selectedSeller, action: 'deactivate' });setSelectedSeller(null);}}
                ...
                {(selectedSeller.status === 'suspended' || selectedSeller.status === 'deactivated') &&
              <button
                onClick={() => {setActionModal({ seller: selectedSeller, action: 'reactivate' });setSelectedSeller(null);}}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-card rounded-2xl border border-border w-full max-w-md shadow-2xl p-6">
            ...
            actionModal.action === 'reactivate' ? 'bg-success/10' : actionModal.action === 'suspend' ? 'bg-warning/10' : 'bg-error/10'}`
            }>
                <Icon
                name={actionModal.action === 'reactivate' ? 'CheckCircleIcon' : actionModal.action === 'suspend' ? 'PauseCircleIcon' : 'NoSymbolIcon'}
                size={20}
                className={actionModal.action === 'reactivate' ? 'text-success' : actionModal.action === 'suspend' ? 'text-warning' : 'text-error'} />
              
              </div>
              <div>
                <h3 className="font-800 text-foreground capitalize">
                  {actionModal.action === 'reactivate' ? 'Reactivate' : actionModal.action === 'suspend' ? 'Suspend' : 'Deactivate'} Seller
                </h3>
                <p className="text-xs text-muted-foreground">{actionModal.seller.name}</p>
              </div>
            </div>
            <p className="text-sm text-muted-foreground mb-4">
              {actionModal.action === 'suspend' ? 'This will temporarily suspend the seller. They cannot accept new orders but existing orders will continue.' :
            actionModal.action === 'deactivate' ? 'This will permanently deactivate the seller account. All active listings will be hidden. This action requires manual review to reverse.' : 'This will restore the seller\'s account and allow them to accept new orders again.'}
            </p>
            <div className="mb-4">
              <label className="text-xs font-700 text-foreground block mb-1.5">
                Reason {actionModal.action !== 'reactivate' && <span className="text-error">*</span>}
              </label>
              <textarea
              value={actionReason}
              onChange={(e) => setActionReason(e.target.value)}
              placeholder={actionModal.action === 'reactivate' ? 'Optional note...' : 'Provide reason for this action...'}
            ...
            <div className="flex items-center gap-2">
              ...
              actionModal.action === 'reactivate' ? 'bg-success text-white hover:bg-green-700' :
              actionModal.action === 'suspend' ? 'bg-warning text-white hover:bg-amber-600' : 'bg-error text-white hover:bg-red-700'}`
              }>
              
                Confirm {actionModal.action === 'reactivate' ? 'Reactivation' : actionModal.action === 'suspend' ? 'Suspension' : 'Deactivation'}
              </button>
              <button
