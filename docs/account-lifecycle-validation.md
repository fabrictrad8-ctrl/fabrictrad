# Account lifecycle validation

This temporary branch validates server-authoritative login, resumable buyer and seller onboarding, and OTP-protected account deletion against the repository's permanent CI workflows.

Final release-candidate validation includes exact production migration versions, service-role-only destructive operations, and least-privilege onboarding draft access.
