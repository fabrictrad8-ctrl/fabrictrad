-- Recover or complete the authenticated user's commerce profiles without
-- relying on service-role credentials in the OAuth callback.
create or replace function public.ensure_current_account_profile(
  p_requested_role text default 'buyer'
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  select * into auth_record from auth.users where id = current_user_id;
  if not found then
  ...
  metadata_phone := nullif(
    regexp_replace(coalesce(auth_record.raw_user_meta_data->>'phone', ''), '\D', '', 'g'),
    ''
  );
  ...
  select * into profile_record from public.user_profiles where id = current_user_id;
  ...
  if not found then
    ...
    set email = normalized_email,
        ...
        phone = coalesce(public.user_profiles.phone, metadata_phone),
        updated_at = now()
  ...
  return jsonb_build_object(
    ...
    'canSell', should_sell,
    'phonePresent', nullif(regexp_replace(coalesce(profile_record.phone, ''), '\D', '', 'g'), '') is not null
  );
end;
$function$;

revoke all on function public.ensure_current_account_profile(text) from public;
revoke all on function public.ensure_current_account_profile(text) from anon;
grant execute on function public.ensure_current_account_profile(text) to authenticated;
grant execute on function public.ensure_current_account_profile(text) to service_role;
