-- Repair legacy seller applications and expose an authenticated verification-state summary.
...
select
  sp.user_id,
  coalesce(nullif(sp.seller_ref, ''), 'FT-SLR-' || upper(left(replace(sp.user_id::text, '-', ''), 12))),
  up.phone,
  up.full_name,
  up.email,
  coalesce(nullif(sp.legal_business_name, ''), nullif(up.business_name, ''), up.full_name),
  sp.business_type,
  coalesce(sp.pickup_address ->> 'city', up.city),
  coalesce(sp.pickup_address ->> 'state', up.state),
  coalesce(sp.pickup_address ->> 'pincode', up.pincode),
  coalesce(sp.pickup_address ->> 'line1', up.address_line1),
  sp.gstin,
  sp.pan,
  coalesce(sp.gstin_verified, false),
  false,
  'pending',
...
set verification_status = 'phone_unverified'::public.seller_status,
    updated_at = now()
from public.user_profiles up
where up.id = sp.user_id
  and coalesce(up.phone_verified, false) = false
  and sp.verification_status in (
    'registration_started'::public.seller_status,
    'phone_unverified'::public.seller_status,
    'profile_incomplete'::public.seller_status,
    'documents_submitted'::public.seller_status,
    'automated_review'::public.seller_status,
    'manual_review'::public.seller_status,
    'additional_docs_required'::public.seller_status
  );

create or replace function public.sync_confirmed_auth_phone_to_profile()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if new.phone_confirmed_at is not null then
    normalized_phone := right(regexp_replace(new.phone, '[^0-9]', '', 'g'), 10);
    update public.user_profiles
...
revoke all on function public.sync_confirmed_auth_phone_to_profile() from public, anon, authenticated;

drop trigger if exists sync_confirmed_auth_phone_to_profile on auth.users;
create trigger sync_confirmed_auth_phone_to_profile
...
execute function public.sync_confirmed_auth_phone_to_profile();
...
begin
  ...
  v_profile_complete :=
    ...
    and nullif(btrim(coalesce(v_user.phone, '')), '') is not null
  ...
  if not coalesce(v_user.phone_verified, false) then
    v_status := 'phone_unverified'::public.seller_status;
    v_next_action := 'verify_phone';
  ...
  update public.seller_registrations
  set phone = coalesce(nullif(phone, ''), v_user.phone),
  ...
  return jsonb_build_object(
    ...
    'phonePresent', nullif(btrim(coalesce(v_user.phone, '')), '') is not null,
    'phoneVerified', coalesce(v_user.phone_verified, false),
    'phone', v_user.phone,
